...
void Classifier::Visualization(cv::Mat prediction_map, string LUT_file) {
  ...
  for (int i = 0; i < output_image.rows; ++i)
  {
      ...
      {
          pixel[j][2] = pixel[j][2]*100;
          pixel[j][1] = pixel[j][1]*100;
          pixel[j][2] = pixel[j][0]*100;
      }
  }
...
int main(int argc, char** argv) {
  ...
  string filename = "/home/ubuntu/Downloads/aerial.mp4";
  ...
  for(; ;){
   ...
   if(frame.empty())
            break;
   classifier.Predict(frame, LUT_file);
   cv::resize(frame, frame,  cv::Size(640,360));
   cv::imshow("w", frame);
   cv::waitKey(30); // waits to display frame
  }
}
#else
int main(int argc, char** argv) {
